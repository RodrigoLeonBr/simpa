...
function historicoSorted(data: ContratoDashboard): HistoricoMensal[] {
  return [...(data.modulos.atencao_primaria_esus.historico_mensal || [])].sort((a, b) =>
...
function countMetasAtingidas(data: ContratoDashboard): number | null {
  const items = data.modulos.financiamento_metas.indicadores;
...
export function buildPainelKpis(data: ContratoDashboard): PainelKpi[] {
  ...
  const atendimentos = data.kpis_gerais.total_atendimentos_aps;
  const odonto = data.kpis_gerais.atendimentos_odonto;
  const coletivas = data.kpis_gerais.total_participantes_coletivos;
...
export function buildUnitTable(data: ContratoDashboard, unidades: Unidade[]): UnitTableRow[] {
  ...
  return unidades
    ...
    .map((unit) => {
      const isActive = unit.nome === activeUnit;
      const atendimentos = isActive ? data.kpis_gerais.total_atendimentos_aps : null;
      const odonto = isActive ? data.kpis_gerais.atendimentos_odonto : null;
...
export function buildModuleStatuses(data: ContratoDashboard): ModuleStatus[] {
  const siaStatus = data.modulos.ambulatorial_sia.status_conexao;
  const sihdStatus = data.modulos.hospitalar_sihd.status_importacao;
